...
DEFAULT_GROUP_ID = -1002470503583  # Default group ID
max_duration = 300  # Default max attack duration

# Group Management
allowed_groups = load_allowed_groups() or {DEFAULT_GROUP_ID}

# Reseller System
resellers = load_resellers()  # Stores reseller user IDs
...
owner_keyboard = [
    ['Start', 'Attack', 'Stats', 'Add User', 'Remove User', 'List Users'],
...
def is_authorized_user(update: Update):
    return is_owner(update) or is_reseller(update) or update.effective_user.id in authorized_users
...
async def handle_button_click(update: Update, context: CallbackContext):
    ...
    elif query == 'Remove User':
        await update.message.reply_text('⚠️ Use /removeuser <user_id> command in this chat.', parse_mode='Markdown')
    elif query == 'List Users':
        await list_users(update, context)
    elif query == 'Add User':
        await update.message.reply_text('⚠️ Use /adduser <user_id> command in this chat.', parse_mode='Markdown')
    elif query == 'Stats':
...
async def show_stats(update: Update, context: CallbackContext):
    if not is_owner(update):
        await update.message.reply_text("❌ *Only the owner can view bot stats!*", parse_mode='Markdown')
        return

    total_resellers = len(resellers)
    total_allowed_groups = len(allowed_groups)
    total_feedback_pending = len(users_pending_feedback)
...
# Add User Command (Manual Private Access)
async def add_user(update: Update, context: CallbackContext):
    if not is_owner(update):
        await update.message.reply_text("❌ *Only the owner can add users!*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        await update.message.reply_text(f"✅ *User {user_id} has been authorized to use the bot in private chat!*", parse_mode='Markdown')
    except ValueError:
        await update.message.reply_text("❌ *Invalid user ID! Please enter a numeric ID.*", parse_mode='Markdown')


# Remove User Command
async def remove_user(update: Update, context: CallbackContext):
    if not is_owner(update):
        await update.message.reply_text("❌ *Only the owner can remove users!*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if user_id in authorized_users:
            ...
            save_authorized_users()
            await update.message.reply_text(f"✅ *User {user_id} has been removed from authorized users.*", parse_mode='Markdown')
        else:
...
async def list_users(update: Update, context: CallbackContext):
    if not is_owner(update):
        await update.message.reply_text("❌ *Only the owner can list authorized users!*", parse_mode='Markdown')
        return

    if not authorized_users:
        await update.message.reply_text("ℹ️ *No users are currently authorized.*", parse_mode='Markdown')
        return
